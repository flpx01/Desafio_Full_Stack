...
import { registrarCompra } from '@/service/compraService';
...
const confirmarCompra = async (forma: string) => {
  try {
    const payload = {
      total: total.value,
      forma_pagamento: forma.toLowerCase(),
      itens: cart.itensParaEnvio, // vem do getter do store
    };
    console.log('Payload que será enviado:', payload);

    console.log('Dados enviados para o backend:', JSON.stringify(payload, null, 2)); // para debug

    await registrarCompra(payload); // envia para o backend

    cart.finalizarCompra(forma); // salva forma de pagamento também
